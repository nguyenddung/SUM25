...
class Draw_button(Init):
    ...
    def BACKGROUP_BUTTON(self):
        pygame.draw.rect(
            self.screen,
            self.color,
            (self.x_init, self.y_init + 40, self.x_max - 195, self.width + 10),
        )
    ...
    def button_result_1(self):
        pygame.draw.rect(
            self.screen,
            self.GREY,
            (self.x_init + 5, self.y_init + 45, self.Length + 20, self.width),
        )
    ...
    def button_clearn_line_1(self):
        pygame.draw.rect(
            self.screen,
            self.GREY,
            (self.x_init + 130, self.y_init + 45, self.Length + 20, self.width),
        )
    ...
    def button_error(self):
        pygame.draw.rect(
            self.screen,
            self.GREY,
            (self.x_init + 255, self.y_init + 45, self.Length + 80, self.width),
        )
    ...
    def button_result_2(self):
        pygame.draw.rect(
            self.screen,
            self.GREY,
            (self.x_init + 440, self.y_init + 45, self.Length + 30, self.width),
        )
    ...
    def button_clearn_line_2(self):
        pygame.draw.rect(
            self.screen,
            self.GREY,
            (self.x_init + 575, self.y_init + 45, self.Length + 20, self.width),
        )
    ...
    def button_reset(self):
        pygame.draw.rect(
            self.screen,
            self.GREY,
            (self.x_init + 700, self.y_init + 45, self.Length, self.width),
        )
    ...
    def Show_when_error(self):
        pygame.draw.rect(
            self.screen,
            self.GREY,
            (self.x_init + 805, self.y_init + 45, self.Length + 395, self.width),
        )
    ...
    def font_note(self):
        pygame.draw.rect(
            self.screen,
            self.GREY,
            (self.x_init + 1250, self.y_init - 120, self.Length + 80, self.width + 70),
        )
    ...
    def show_name_button(self):
        ...
        for i in range(len(fonts)):
            for index in range(len(value)):
                self.screen.blit(
                    fonts[i][index], (list_value[index][0], list_value[index][1])
                )
...
class Draw_point:
    ...
    def draw_points(self):
        ...
        for i in range(len(point)):
            pygame.draw.circle(
                self.screen, self.RED, (point[i][0] + 50, 700 - point[i][1]), 7
            )
    ...
    def draw_lr_recipe(self):
        ...
        for i in range(len(list_value_result)):
            pygame.draw.line(
                self.screen,
                self.BLUE,
                (list_value_result[i][0] * 10 + 50, 700 - list_value_result[i][1] * 10),
                (list_value_result[i][2] * 10 + 50, 700 - list_value_result[i][3] * 10),
                4,
            )
    ...
    def draw_lr_library(self):
        ...
        for i in range(len(Const)):
            pygame.draw.line(
                self.screen,
                self.BLACK,
                (Const[i][0] * 10 + 50, 700 - Const[i][1] * 10),
                (Const[i][2] * 10 + 50, 700 - Const[i][3] * 10),
                4,
            )


def Draw_interface(
    screen, running, clock, COLORS, x_init, y_init, x_max, length, width
):
    ...
    while running:
        ...
        draw_button = Draw_button(
            screen, COLORS.BLACK, x_init, y_init, x_max, length, width, COLORS.GREY
        )
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if (x_init + 5) <= x_mouse <= (length + x_init + 25) and (
                    y_init + 45
                ) <= y_mouse <= (y_init + 85):
                    list_value_result = []
                    list_error_lr = []
                    ...
                    try:
                        ...
                        (x1, y1, x2, y2, a, b) = calc_linear_ressgrion(
                            value_X,
                            value_Y,
                            (list_x_min_max[0] - 50) / 10,
                            (list_x_min_max[1] - 50) / 10,
                        )
                ...
                if (x_init + 130) <= x_mouse <= (x_init + length + 150) and (
                    y_init + 45
                ) <= y_mouse <= (y_init + width + 45):
                    list_value_result = []
                    list_distance = []

                # Tính đường thẳng bằng thư viện
                if (x_init + 440) <= x_mouse <= (x_init + length + 485) and (
                    y_init + 45
                ) <= y_mouse <= (y_init + width + 45):
                    error = 0
                    try:
                ...
                if (x_init + 575) <= x_mouse <= (x_init + length + 595) and (
                    y_init + 45
                ) <= y_mouse <= (y_init + width + 45):
                    list_value = []
                    list_distance = []
                ...
                if (x_init + 700) <= x_mouse <= (x_init + length + 700) and (
                    y_init + 45
                ) <= y_mouse <= (y_init + width + 45):
                    points = []
                    list_value_result = []
                ...
                if (x_init + 1250) <= x_mouse <= (x_init + length + 1350) and (
                    y_init - 120
                ) <= y_mouse <= (y_init + width - 50):
                    points = []
